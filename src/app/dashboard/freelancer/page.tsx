...
const IncomeChart = dynamic(() => import("@/components/Charts").then((m) => ({ default: m.IncomeChart })), { ssr: false })
const ProjectsChart = dynamic(() => import("@/components/Charts").then((m) => ({ default: m.ProjectsChart })), { ssr: false })
const CategoryChart = dynamic(() => import("@/components/Charts").then((m) => ({ default: m.CategoryChart })), { ssr: false })
...
export default function FreelancerDashboard() {
  ...
  useEffect(() => {
    if (!user || profile?.role !== "freelancer") { setLoading(false); return }
    const load = async () => {
  ...
  const handleOrderAction = useCallback(async (orderId: string, status: string) => {
    ...
    if (error) { addToast("Error al actualizar orden", "error"); return }
    setOrders(prev => prev.map(o => o.id === orderId ? { ...o, status } as Order : o))
    addToast(`Orden ${statusLabels[status] ?? status}`, "success")
  }, [supabase, addToast])
  ...
  const activeOrders = useMemo(() => orders.filter((o) => o.status === "in_progress").length, [orders])
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12">
      <AnimatedSection>
        <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-8 gap-4">
          <div>
            <h1 className="text-3xl sm:text-4xl font-bold text-foreground">Dashboard Freelancer</h1>
            <p className="mt-2 text-muted-fg">Bienvenido de vuelta, {profile?.name}. Gestiona tus servicios y órdenes.</p>
          </div>
          <Link
            href="/services/new"
          ...
          <div className="rounded-2xl border border-card-border bg-card-bg overflow-hidden hover:shadow-md transition-shadow">
            <div className="p-6 border-b border-card-border">
              <h2 className="font-semibold text-foreground">
                {orders.length > 0 ? "Órdenes Recibidas" : "No tienes órdenes recibidas aún"}
              </h2>
            </div>
            {orders.length > 0 ? (
                          ...
                          <div className="flex gap-1.5">
                            {order.status === "pending" && (
                              <>
                                <button
                                  ...
                                  className="text-xs px-2.5 py-1 rounded-lg bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400 hover:opacity-80 font-medium"
                                >
                                  Aceptar
                                </button>
                                <button
                            ...
                            {order.status === "accepted" && (
                              <button
                                ...
                                className="text-xs px-2.5 py-1 rounded-lg bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400 hover:opacity-80 font-medium"
                              >
                                Empezar
                              </button>
                            )}
      ...
      {activeTab === "services" && (
        <AnimatedSection key="services">
          <div className="flex items-center justify-between mb-4">
            <h2 className="font-semibold text-foreground">
              {myServices.length > 0 ? "Mis Servicios" : "No has publicado servicios aún"}
            </h2>
            <Link
              href="/services/new"
          ...
          {myServices.length > 0 ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {myServices.map((service) => (
                <div key={service.id} className="p-5 rounded-2xl border border-card-border bg-card-bg hover:shadow-md transition-shadow">
