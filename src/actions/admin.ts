...
export async function updateCommissionRate(rate:number){if(rate<0||rate>.5)throw new Error("Comisión inválida");const {pool,actorId}=await admin();await pool.request().input("value",sql.NVarChar(500),String(rate)).query(`UPDATE dbo.platform_settings SET value=@value,updated_at=SYSUTCDATETIME() WHERE[key]='commission_rate'`);await logAudit(pool,actorId,"update_commission_rate","platform_settings","commission_rate",null,{rate})}

// ─── Ban / unban ──────────────────────────────────────────────────────────
export async function banUser(id:string,reason?:string){
  const {pool,actorId}=await admin()
...
export async function resolveDispute(orderId:string,resolution:"completed"|"cancelled"){
  const {pool,actorId}=await admin()
