...
const categoriesMap: Record<string, CategoryInfo> = {
  "desarrollo-web": { icon: Globe, gradient: "from-blue-600 to-blue-500", name: "Desarrollo Web" },
  "diseno-grafico": { icon: Palette, gradient: "from-rose-600 to-rose-500", name: "Diseño Gráfico" },
  "marketing-digital": { icon: TrendingUp, gradient: "from-amber-600 to-amber-500", name: "Marketing Digital" },
  redaccion: { icon: PenTool, gradient: "from-emerald-600 to-emerald-500", name: "Redacción y Traducción" },
  "video-animacion": { icon: Clapperboard, gradient: "from-red-600 to-red-500", name: "Video y Animación" },
  "musica-audio": { icon: Music, gradient: "from-violet-600 to-violet-500", name: "Música y Audio" },
  "programacion-tech": { icon: Code, gradient: "from-indigo-600 to-indigo-500", name: "Programación y Tech" },
  consultoria: { icon: Briefcase, gradient: "from-slate-600 to-slate-500", name: "Consultoría" },
}
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
          {categories.map((category, index) => {
            const cfg = categoriesMap[category.slug] || { icon: Briefcase, gradient: "from-slate-600 to-slate-500", name: category.name }
                ...
                <Link href={`/marketplace?category=${category.slug}`} className="group block">
                  <TiltCard tiltDegree={4} glare={true} scale={1.03}>
                    <div className="neo-card flex min-h-44 flex-col items-center justify-center gap-3 rounded-lg p-5 text-center transition-transform duration-300 group-hover:-translate-y-1">
                      <div className={`grid h-14 w-14 place-items-center rounded-lg bg-gradient-to-br ${cfg.gradient} text-white shadow-md shadow-primary/10`}>
